...
def sample_diffusion_ligand_decomp(
    ...
    atom_prior_probs=None,
    bond_prior_probs=None,
    arms_natoms_config=None,
    scaffold_natoms_config=None,
    natoms_config=None,
    ...
    for i in tqdm(range(num_batch)):
        ...
        if prior_mode == "subpocket":
            # init ligand pos
            arm_pocket_sizes = [
                atom_num.get_space_size(
                    data.protein_pos[data.pocket_atom_masks[arm_i]]
                    .detach()
                    .cpu()
                    .numpy()
                )
            ...
            scaffold_pocket_size = atom_num.get_space_size(
                data.protein_pos.detach().cpu().numpy()
            )
            arm_centers = [
                data.protein_pos[pocket_mask].mean(0)
                for pocket_mask in data.pocket_atom_masks
            ]  # [num_arms, 3]
            scaffold_center = data.protein_pos.mean(0)
            data.ligand_decomp_centers = torch.cat(
                arm_centers + [scaffold_center], dim=0
            )
            ...
            for data_idx in range(n_data):
                ...
                for arm_i in range(data.num_arms):
                    if num_atoms_mode == "prior":
                        arm_num_atoms = atom_num.sample_atom_num(
                            arm_pocket_sizes[arm_i], arms_natoms_config
                        ).astype(int)
                        ...
                        arm_num_atoms = int(
                            (data.ligand_atom_mask == arm_i).sum().item() + inc
                        )
                    else:
                        raise ValueError
                ...
                if num_atoms_mode == "prior":
                    scaffold_num_atoms = atom_num.sample_atom_num(
                        scaffold_pocket_size, scaffold_natoms_config
                    ).astype(int)
                elif num_atoms_mode == "ref":
                    ...
                    scaffold_num_atoms = int(
                        (data.ligand_atom_mask == -1).sum().item() + inc
                    )
                else:
                    raise ValueError
                init_scaffold_pos = scaffold_center + torch.randn(
                    [scaffold_num_atoms, 3]
                )
                init_ligand_pos.append(init_scaffold_pos)
                ligand_atom_mask += [-1] * scaffold_num_atoms
                n_atoms += scaffold_num_atoms
                new_data = data.clone()
                new_data.ligand_atom_mask = torch.tensor(
                    ligand_atom_mask, dtype=torch.long
                )
                ...
                if getattr(new_data, "ligand_fc_bond_index") is not None:
                    if bond_prior_probs is not None:
                        new_data.ligand_fc_bond_type = torch.multinomial(
                            torch.from_numpy(bond_prior_probs.astype(np.float32)),
                            new_data.ligand_fc_bond_index.size(1),
                            replacement=True,
                        )
                    else:
                        uniform_logits = torch.zeros(
                            new_data.ligand_fc_bond_index.size(1),
                            model.num_bond_classes,
                        )
                        new_data.ligand_fc_bond_type = log_sample_categorical(
                            uniform_logits
                        )
            ...
            for data_idx in range(n_data):
                ...
                for arm_i in range(data.num_arms):
                    ...
                    init_arm_pos = arm_centers[arm_i] + torch.randn(
                        [arm_num_atoms, 3]
                    ) * arms_stds[arm_i, :].unsqueeze(0)
                    init_ligand_pos.append(init_arm_pos)
                    ligand_atom_mask += [arm_i] * arm_num_atoms
                    n_atoms += arm_num_atoms

                scaffold_num_atoms = (
                    int(old_data.scaffold_prior[0][0])
                    if len(old_data.scaffold_prior) == 1
                    else 0
                )
                init_scaffold_pos = scaffold_center + torch.randn(
                    [scaffold_num_atoms, 3]
                ) * scaffold_std.unsqueeze(0)
            ...
            for data_idx in range(n_data):
                ...
                if num_atoms_mode == "stat":
                    arm_natoms, arms_stds = natoms_sampler.sample_arm_natoms(
                        arm_centers, data.protein_pos
                    )
                    if len(data.scaffold_prior) > 0:
                        scaffold_center = [p[1] for p in data.scaffold_prior][0]
                        scaffold_natoms, scaffold_std = (
                            natoms_sampler.sample_sca_natoms(
                                scaffold_center,
                                arm_centers,
                                arms_stds,
                                data.protein_pos,
                        ...
                        scaffold_center = data.protein_pos.mean(0)
                        scaffold_natoms, scaffold_std = 0, torch.tensor([0.0])
                ...
                for arm_i in range(data.num_arms):
                    if num_atoms_mode == "old":
                        ...
                        _m = 12.41
                        _b = -4.98
                        arm_num_atoms_lower = torch.clamp(
                            np.floor((_m - 2.0) * arm_std[0] + _b), min=2
                        ).long()
                        arm_num_atoms_upper = torch.clamp(
                            np.ceil((_m + 3.0) * arm_std[0] + _b), min=2
                        ).long()
                        arm_num_atoms = int(
                            torch.randint(
                                low=arm_num_atoms_lower,
                                high=arm_num_atoms_upper + 1,
                                size=(1,),
                            )
                        )
                ...
                if num_atoms_mode == "old":
                    _m = 12.41
                    _b = -4.98
                    scaffold_num_atoms_lower = torch.clamp(
                        np.ceil((_m - 2.0) * scaffold_std[0] + _b), min=2
                    ).long()
                    scaffold_num_atoms_upper = torch.clamp(
                        np.ceil((_m + 3.0) * scaffold_std[0] + _b), min=2
                    ).long()
                    scaffold_num_atoms = int(
                        torch.randint(
                            low=scaffold_num_atoms_lower,
                            high=scaffold_num_atoms_upper + 1,
                            size=(1,),
                        )
                    )
                elif num_atoms_mode == "v2":
        ...
        if atom_prior_probs is not None:
            init_ligand_v = torch.multinomial(
                torch.from_numpy(atom_prior_probs.astype(np.float32)),
                len(batch_ligand),
                replacement=True,
            ).to(device)
        else:
        ...
        full_batch_protein = (
            torch.arange(n_data).repeat_interleave(len(full_protein_pos)).to(device)
        )
        ...
        if getattr(model, "bond_diffusion", False):
            ...
            ligand_num_bonds = scatter_sum(
                torch.ones_like(batch.ligand_fc_bond_type_batch),
                batch.ligand_fc_bond_type_batch,
            ).tolist()
            cum_bonds = np.cumsum([0] + ligand_num_bonds)
            all_pred_bond_index += [
                ligand_bond_index_array[:, cum_bonds[k] : cum_bonds[k + 1]]
                - ligand_cum_atoms[k]
                for k in range(n_data)
            ]
            all_pred_bond += [
                ligand_bond_array[cum_bonds[k] : cum_bonds[k + 1]]
                for k in range(n_data)
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--beta_prior_path", type=str, default="./pregen_info/beta_priors"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prior_mode", type=str, choices=["subpocket", "ref_prior", "beta_prior"]
    )
    parser.add_argument(
        "--num_atoms_mode",
        type=str,
    ...
    parser.add_argument(
        "--bp_num_atoms_mode", type=str, choices=["old", "stat", "v2"], default="v2"
    )
    ...
    parser.add_argument(
        "--wandb_name",
        type=str,
        default="visualizing sampling process for guided decompdiff on pocket 50",
    )
    ...
    ckpt_path = (
        args.ckpt_path if args.ckpt_path is not None else config.model.checkpoint
    )
    ...
    prior_mode = (
        config.sample.prior_mode if args.prior_mode is None else args.prior_mode
    )
    init_transform_list = [
    ...
    # classifier first
    print(
        f"protein dim {protein_featurizer.protein_feature_dim + decomp_indicator.protein_feature_dim}, ligand dim {ligand_featurizer.ligand_feature_dim + decomp_indicator.ligand_feature_dim}"
    )
    print(
        f"prior atom {ligand_featurizer.atom_types_prob}, prior bond {ligand_featurizer.bond_types_prob}"
    )
    classifier = clsfer(
        config.model,
    ...
    logger.info(
        f"Successfully load arms num atoms config from {config.sample.arms_num_atoms_config}"
    )
    with open(config.sample.scaffold_num_atoms_config, "rb") as f:
        scaffold_num_atoms_config = pickle.load(f)
    logger.info(
        f"Successfully load scaffold num atoms config from {config.sample.scaffold_num_atoms_config}"
    )
    ...
    for data_id in list(range(args.start_data_id, args.end_data_id)):
        ...
        full_protein = PDBProtein(
            os.path.join(args.ori_data_path, data.src_protein_filename)
        )
        full_protein_pos = torch.from_numpy(
            full_protein.to_dict_atom()["pos"].astype(np.float32)
        )
        ...
        if prior_mode == "subpocket":
            ...
            utils_prior.substitute_golden_prior_with_beta_prior(
                data, beta_prior_path=beta_prior_path
            )
        ...
        raw_results = sample_diffusion_ligand_decomp(
            ...
            num_atoms_mode=args.bp_num_atoms_mode
            if prior_mode == "beta_prior"
            else args.num_atoms_mode,
        ...
        for r in raw_results:
            results.append({**r, "ligand_filename": index["src_ligand_filename"]})
