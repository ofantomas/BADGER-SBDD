...
class DecompScorePosNet3D(nn.Module):
    def __init__(
        ...
        num_classes,
        prior_atom_types=None,
        prior_bond_types=None,
    ):
        super().__init__()
        self.config = config
        ...
        self.atom_type_trans = DiscreteTransition(
            config.v_beta_schedule,
            self.num_timesteps,
            s=config.v_beta_s,
            num_classes=self.num_classes,
            prior_probs=prior_atom_types,
        )
        ...
        if self.add_prior_node:
            self.prior_std_expansion = GaussianSmearing(
                0.0, 5.0, num_gaussians=20, fix_offset=False
            )
        ...
        if self.bond_diffusion:
            self.distance_expansion = GaussianSmearing(
                0.0, 5.0, num_gaussians=config.num_r_gaussian, fix_offset=False
            )
    ...
    def forward(
        ...
        batch_prior,
        prior_group_idx,
        ligand_fc_bond_index,
        init_ligand_fc_bond_type,
        ...
        if self.config.node_indicator:
            if self.add_prior_node:
                pro_ind = (
                    torch.tensor([1, 0, 0])
                    .unsqueeze(0)
                    .repeat(len(h_protein), 1)
                    .to(h_protein)
                )
                lig_ind = (
                    torch.tensor([0, 1, 0])
                    .unsqueeze(0)
                    .repeat(len(init_ligand_h), 1)
                    .to(h_protein)
                )
                prior_ind = (
                    torch.tensor([0, 0, 1])
                    .unsqueeze(0)
                    .repeat(len(prior_h), 1)
                    .to(h_protein)
                )
        ...
        if self.refine_net_type == "uni_o2_bond":
            bond_type = F.one_hot(
                init_ligand_fc_bond_type, num_classes=self.num_bond_classes
            ).float()
            ...
            outputs = self.refine_net(
                ...
                group_idx=group_idx_all,
                bond_index=bond_index_in_all,
                bond_type=init_ligand_fc_bond_type,
                mask_ligand=mask_ligand,
                mask_ligand_atom=mask_ligand_atom,  # dummy node is marked as 0
                batch=batch_all,
                return_all=return_all,
        ...
        final_ligand_pos, final_ligand_h = (
            final_pos[mask_ligand_atom],
            final_h[mask_ligand_atom],
        )
        final_ligand_v = self.v_inference(final_ligand_h)
        preds = {"pred_ligand_pos": final_ligand_pos, "pred_ligand_v": final_ligand_v}
        ...
        if self.bond_diffusion:
            ...
            if self.bond_net_type == "pre_att":
                ...
                dist = torch.norm(
                    final_pos[dst] - final_pos[src], p=2, dim=-1, keepdim=True
                )
    ...
    def get_diffusion_loss(
        ...
        ligand_pos_perturbed = (
            a_pos.sqrt() * (ligand_pos - batch_noise_centers)
            + (1.0 - a_pos).sqrt() * pos_noise * batch_noise_stds
            + batch_noise_centers
        )
        # Vt = a * V0 + (1-a) / K
        log_ligand_v0 = index_to_log_onehot(ligand_v, self.num_classes)
        ligand_v_perturbed, log_ligand_vt = self.atom_type_trans.q_v_sample(
            log_ligand_v0, time_step, batch_ligand
        )

        if self.bond_diffusion:
            log_ligand_b0 = index_to_log_onehot(
                ligand_fc_bond_type, self.num_bond_classes
            )
        ...
        log_v_model_prob = self.atom_type_trans.q_v_posterior(
            log_ligand_v_recon, log_ligand_vt, time_step, batch_ligand
        )
        log_v_true_prob = self.atom_type_trans.q_v_posterior(
        ...
        if self.bond_diffusion:
            ...
            kl_b = self.compute_v_Lt(
                log_v_model_prob=log_b_model_prob,
                log_v0=log_ligand_b0,
                log_v_true_prob=log_b_true_prob,
                t=time_step,
        ...
        if self.loss_pos_type == "mse":
            ...
            if self.model_mean_type == "C0":
                ...
                raise ValueError
            loss_pos = scatter_mean(
                (((pred - target) ** 2) / (batch_noise_stds**2)).sum(-1),
                batch_ligand,
                dim=0,
            )
    ...
    def sample_diffusion(
        ...
        bt_pred_traj = []
        ligand_pos, ligand_v, ligand_bond = (
            init_ligand_pos,
            init_ligand_v,
            init_ligand_fc_bond_type,
        )
                ...
                for drift in energy_drift_opt:
                    ...
                    if drift["type"] == "center_prox":
                        with torch.enable_grad():
                            energy = guidance.compute_center_prox_loss(
                                xt, prior_centers[ligand_decomp_batch]
                            )
                        ...
                        with torch.enable_grad():
                            ...
                            if n_valid > 0:
                                ...
                                if drift.get("scale", False):  # coef -> 0 when T -> 0
                                    energy_grad *= extract(
                                        self.pos_score_coef, t, batch_ligand
                                    )
                            else:
                                energy_grad = 0.0
                        ...
                        if i < drift["start_time"] and i >= drift["end_time"]:
                            energy_grad = guidance.compute_conf_drift(
                                pos_model_mean, ligand_v_next, batch_ligand
                            )
