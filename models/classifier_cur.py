...
class Classifier(nn.Module):
    def __init__(
        self, config, protein_atom_feature_dim, ligand_atom_feature_dim, device="cpu"
    ):
        super().__init__()
        self.config = config
        self.device = device
    ...
    def forward(
        ...
        final_pos, final_h = (
            outputs["x"],
            outputs["h"],
        )  # combined graph between ligand and protein pocket, use this as output head feeding
        final_ligand_pos, final_ligand_h = (
            final_pos[mask_ligand],
            final_h[mask_ligand],
        )  # extract the ligand graph
