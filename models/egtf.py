...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, num_heads, num_ffn, dropout_r, act_fn_ecd):
        ...
        self.ffn = MLP(
            d_model=d_model, num_ffn=num_ffn, act_fn=act_fn_ecd, dropout_r=dropout_r
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = qkv.chunk(
            3, dim=-1
        )  # breakup using the last dimension, each are 30 x 8 x 200 x 64
...
class LayerNormalization(nn.Module):
    def __init__(self, parameters_shape, eps=1e-8):
        ...
        )  # learnable parameter "std" (512,)
        self.beta = nn.Parameter(
            torch.zeros(parameters_shape)
        )  # learnable parameter "mean" (512,)

    def forward(self, inputs):
        dims = [-(i + 1) for i in range(len(self.parameters_shape))]
        mean = inputs.mean(
            dim=dims, keepdim=True
        )  # eg. for (30, 200, 512) inputs, mean -> (30, 200, 1)
...
class EGTF(nn.Module):
    def __init__(
        ...
        self.encoder_layers = nn.ModuleList(
            [
                EncoderLayer(hidden_dim, num_heads, num_ffn, dropout_r, act_fn_ecd)
    ...
    def _connect_edge(self, x, mask_ligand, batch):
        ...
        if self.cutoff_mode == "knn":
            if debug:
                ...
                edge_index = knn_graph(
                    x, k=self.k, batch=batch, flow="source_to_target"
                )
                # edge_index = edge_index.to(self.device)
                # x = x.to(self.device)
