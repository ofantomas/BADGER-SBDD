...
class FeaturizeLigandAtom(object):
    def __init__(self, mode="basic", prior_types=True):
        ...
        if self.mode == "basic" and self.prior_types:
            self.atom_types_prob = np.array(
                [0.0, 0.6716, 0.1174, 0.1689, 0.01315, 0.01117, 0.01128, 0.00647]
            )
            self.bond_types_prob = np.array(
                [0.9170, 0.0433, 0.00687, 0.000173, 0.03266]
            )
        else:
...
class ComputeLigandAtomNoiseDist(object):
    ...
    def __init__(self, version):
        ...
        print(
            f"{version} prior mode is applied in ComputeLigandAtomNoiseDist transform!"
        )
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        if self.version == "subpocket":
            ...
            for arm_idx, pocket_mask in enumerate(data.pocket_atom_masks):
                if pocket_mask.sum() > 0:
                    ...
                    arm_centers.append(
                        data.ligand_pos[data.ligand_atom_mask == arm_idx].mean(0)
                    )
            ...
            data.ligand_decomp_centers = torch.cat(
                [data.arm_centers, data.scaffold_center], dim=0
            )
            ...
            for arm_id in range(data.num_arms):
                (arm_atom_num, arm_iso_mu, arm_iso_cov, arm_aniso_mu, arm_aniso_cov) = (
                    arms_prior[arm_id]
                )
                if arm_atom_num > 1:
                    tmp_std = (
                        torch.tensor([torch.sqrt(arm_iso_cov[0, 0])])
                        .reshape(1, 1)
                        .expand(-1, 3)
                        .clone()
                    )
                    ...
                    tmp_std = (
                        torch.tensor([min_std]).reshape(1, 1).expand(-1, 3).clone()
                    )
                    arms_std.append(tmp_std)
            ...
            if len(scaffold_prior) > 0:
                ...
                if self.version == "ref_prior":
                    if scaffold_atom_num > 1:
                        tmp_std = (
                            torch.tensor([torch.sqrt(scaffold_iso_cov[0, 0])])
                            .reshape(1, 1)
                            .expand(-1, 3)
                            .clone()
                        )
                        ...
                        raise ValueError(
                            f"scaffold_atom_num = {scaffold_atom_num}, not valid!"
                        )
                    ...
                    try:
                        tmp_std = (
                            torch.tensor([torch.sqrt(scaffold_iso_cov)])
                            .reshape(1, 1)
                            .expand(-1, 3)
                            .clone()
                        )
            ...
            data.ligand_decomp_centers = torch.cat(
                arms_centers + scaffold_centers, dim=0
            )
        ...
        data.arm_num_atoms = torch.tensor(
            [(data.ligand_atom_mask == i).sum() for i in range(data.num_arms)]
        )
        data.scaffold_num_atoms = (data.ligand_atom_mask == -1).sum().unsqueeze(0)
        data.ligand_decomp_num_atoms = torch.cat(
            [data.arm_num_atoms, data.scaffold_num_atoms]
        )
        return data
...
class AddDecompIndicator(object):
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        """
        data.prior_group_idx = torch.LongTensor(
            list(range(data.num_arms)) + [data.num_arms]
        )
        ...
        if self.add_to_ligand:
            ...
            if self.add_ord_feat:
                ...
                data.ligand_atom_aux_feature = torch.cat(
                    [arm_scaffold_index, arm_ind], -1
                )
            else:
                data.ligand_atom_aux_feature = arm_ind

        if self.add_to_protein:
            protein_arm_ind = F.one_hot(
                (data.pocket_atom_masks.sum(0) > 0).long(), num_classes=2
            )
            protein_arm_scaffold_index = torch.zeros(
                [len(data.protein_pos), self.num_classes]
            )
            ...
            # dummy idx now
            data.protein_decomp_group_idx = torch.LongTensor(
                [-1] * len(data.protein_pos)
            )
            if self.add_ord_feat:
                data.protein_atom_feature = torch.cat(
                    [
                        data.protein_atom_feature,
                        protein_arm_scaffold_index,
                        protein_arm_ind,
                    ],
                    -1,
                )
                ...
                data.protein_atom_feature = torch.cat(
                    [data.protein_atom_feature, protein_arm_ind], -1
                )
        return data
...
class FeaturizeLigandBond(object):
    ...
    def __call__(self, data: ProteinLigandData):
        if self.mode == "fc":
            n_atoms = len(
                data.ligand_atom_mask
            )  # only ligand atom mask is reset in beta prior sampling
        ...
        if hasattr(data, "ligand_bond_index") and self.set_bond_type:
            ...
            data.ligand_fc_bond_type = bond_matrix[
                data.ligand_fc_bond_index[0], data.ligand_fc_bond_index[1]
            ]
        return data
